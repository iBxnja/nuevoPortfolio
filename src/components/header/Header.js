...
export function Header() {
  ...
  return (
    <header id="header" className={`w-full h-24 flex items-center justify-center fixed cel:flex ${menuOpen ? 'cel:w-full cel:h-screen' : ''}`}>
      <nav id="navbar" className={`flex lg:gap-6 gap-12 text-white w-7/12 h-10 rounded-full items-center justify-center bg-neutral-900 border-2 border-stone-950 ${menuOpen ? 'cel:w-full cel:h-screen cel:rounded-none cel:flex-col' : 'cel:w-10 cel:absolute cel:top-5 cel:left-5'}`} onClick={toggleMenu}>
